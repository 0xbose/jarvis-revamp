...
export function AgentGroup({
	...
	const currentSubnetStatus =
		currentWorkflowData?.subnets?.[group.subnetIndex]?.status;
	...
	const parseTitleAndStatus = (fullTitle: string) => {
		...
		if (agentMatch) {
			return {
				mainTitle: agentMatch[1],
				statusMessage: agentMatch[2].trim(),
			};
		}
	...
	const getSubnetPrompt = () => {
		...
		return subnet?.prompt && subnet.prompt.trim() !== ""
			? subnet.prompt
			: null;
	};
	...
	const shouldShowFeedbackButtons = (message: any): boolean => {
		...
		if (
			message.sourceId?.includes("feedback") &&
			currentWorkflowData?.subnets
		) {
			const subnetIndex = message.subnetIndex;
			if (
				subnetIndex !== undefined &&
				currentWorkflowData.subnets[subnetIndex]
			) {
				...
				if (subnet.feedbackHistory) {
					...
					const feedbackMatch = message.sourceId.match(
						/feedback_question_(\d+)/
					);
					const feedbackIndex = feedbackMatch
					...
					if (feedbackIndex !== null && subnet.feedbackHistory) {
						...
						if (sortedFeedbackHistory[feedbackIndex]) {
							const feedbackItem =
								sortedFeedbackHistory[feedbackIndex];
	...
	const recentThread =
		group.feedbackThreads.length > 0
			? group.feedbackThreads.reduce((latest, current) =>
					current.feedbackIndex > latest.feedbackIndex
						? current
						: latest
			  )
			: null;
	const historyThreads = group.feedbackThreads.filter(
		(thread) => thread !== recentThread
	);
	const shouldShowHistory = historyThreads.length > 0;

	// Determine if we should show a separate three-dots loader message
	const shouldShowFeedbackLoading =
		hasFeedbackHistory &&
		(currentSubnetStatus === "in_progress" ||
			currentSubnetStatus === "pending");
	...
	return (
		<div className="space-y-4">
			<div
				className={`border rounded-lg transition-all duration-300 overflow-hidden border-border bg-card`}
			>
				{shouldShowHistory ? (
					<Collapsible
						open={isHistoryOpen}
									...
									<div className="text-left">
										...
										{subnetPrompt && (
											<div className="pr-16 mr-4">
												<div
													className="text-sm text-muted-foreground mt-1 italic overflow-hidden"
													style={{
														display: isHistoryOpen
															? "block"
															: "-webkit-box",
														WebkitLineClamp:
															isHistoryOpen
							...
							<div className="p-4 space-y-1">
								{historyThreads.map((thread, index) => (
									<div
										...
										className="border border-border/30 rounded-lg bg-background/50 hover:bg-background/70 transition-colors"
									>
										<FeedbackThread
											thread={thread}
											isLast={
												index ===
												historyThreads.length - 1
											}
											currentWorkflowData={
												currentWorkflowData
											}
                    ...
                    <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200 animate-pulse">
                      New Data
								...
								{subnetPrompt && (
									<div className="pr-16 mr-4">
										<div
											className="text-sm text-muted-foreground mt-2 italic overflow-hidden"
											style={{
												display: "-webkit-box",
												WebkitLineClamp: 3,
						...
						<div className="border border-primary/10 rounded-lg bg-card shadow-sm hover:shadow-md transition-shadow">
							<div className="space-y-0">
								...
								{recentThread.question && (
									<>
										{console.log(
											`🔍 Rendering recent question ChatMessage:`,
											{
												questionText:
													recentThread.question.content?.slice(
														0,
														50
													),
												sourceId:
										...
										<ChatMessage
											...
											onFeedbackSubmit={onFeedbackSubmit}
											showFeedbackButtons={shouldShowFeedbackButtons(
												recentThread.question
											)}
											workflowStatus={workflowStatus}
											pollingStoppedAt={pollingStoppedAt}
					...
					{group.mainMessages.map((message, index) => (
						...
						>
							<ChatMessage
								message={message}
								isLast={
									index === group.mainMessages.length - 1 &&
									!recentThread
								}
								onNotificationYes={onNotificationYes}
								onNotificationNo={onNotificationNo}
			...
			{/* Standalone loader outside message card */}
