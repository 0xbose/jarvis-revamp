...
export function AgentChatContainer({
	...
	const handlePromptSubmit = async (
		...
		// // Call workflow start callback
		// if (onWorkflowStart) {
		// 	onWorkflowStart(selectedAgent.id || selectedAgent.address);
		// }
				...
				try {
					...
					if (isMounted) {
						...
						if (err.response?.status === 404) {
							errorMsg = `Agent not found: ${agentAddress}/${"unknown"}`;
							...
							errorMsg = "NFT ID is required but not found";
						} else {
